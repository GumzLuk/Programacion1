...
Uses Dialogs,SysUtils,UnitClasNumero,Math;
const MAXE = 1000;
      VOCALES = ['A','E','I','O','U'];
      CONSO = ['A'..'Z'] - VOCALES;
      SEP_ = [',',':',';','(',')',' '];
...
Type
  Cadena = Class
   ...
   //Procedimiento______________________________________________________________
        ...
        procedure addTexto(Texto:String);
...
procedure Cadena.addTexto(Texto: String);
var
 i,repL,j:Cardinal;
begin
 repL := longitud;
  if(repL = 0)then Inc(repL);
 longitud := longitud + Length(Texto);
 j := 1;
  for i := repL to longitud do
    begin
