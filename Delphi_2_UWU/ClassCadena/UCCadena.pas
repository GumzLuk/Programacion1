...
Type
  Cadena = Class
   ...
   //Procedimiento______________________________________________________________
        ...
        procedure ColocarPalabras(palabras:String);
  ...
  {DecimalTo}
        function RomanoToDecimal(Romano : String):Cardinal;
        function ObtMillarRomano(Romano : String):Cardinal;
        function ObtCentenaRomano(Romano : String):Word;
        function ObtDecenaRomano(Romano : String):Word;
        function ObtUnidadRomano(Romano : String):Word;
        function LiteralToDecimal():Cardinal;
        function UnidadLiteral():Word;
        function ExtraLiteral():Word;
        function DecenaLiteral():Word;
        function CentenaLiteal():Word;
 {...............}
...
{ i := longitud + 1;
 ...
 caracteres[self.longitud] := ' ';
 k := self.longitud + 1 ;
 self.longitud := self.longitud + Length(Text);
 j := 1;
  for i := k to self.longitud do
   begin
...
procedure Cadena.ColocarPalabras(palabras: String);
 var
  i,pos : Word;
  cadenaX : Cadena;
begin
 ...
 i := 1;
  while(i <= CantDPal)do
   begin
          ShowMessage(getPalXPos(i));
     if(getPalXPos(i) = '{?}')then
       begin
         modPalabraXPos(i,cadenaX.getPalXPos(pos));
         pos := pos + 1;
       end;
    i := i +1 ;
   end;
...
begin
 NU := Numero.Create;
 if(Nu.EsFraccion(StrToFloat(N)))then
   begin
     i := 1;
     car := 0;
     k := false;
...
function Cadena.ObtMillarRomano(Romano: String): Cardinal;
 const
  MILLARESROM : array[1..3]of String = ('M','MM','MMM');
  MILLARESROMN : array[1..3]of Word = (1000,2000,3000);
 var
  Nu : Cardinal;
  N : String;
  i : Word;
...
begin
 ...
 Nu := 0;
 i := 1;
  while(i <= longitud)AND(UpCase(caracteres[i]) = 'M')do
    begin
     N := N + UpCase(caracteres[i]);
     i := i + 1;
    end;
  i := 1;
  notIq := false;
   while(i <= 3)AND not(notIq)do
    begin
      if(N = MILLARESROM[i])then
       begin
         Nu := MILLARESROMN[i];
         notIq := true;
       end;
...
function Cadena.ObtCentenaRomano(Romano: String): Word;
 const
   CENTENAROM : array[1..9]of String = ('C','CC','CCC','CD','D','DC','DCC','DCCC','CM');
   CENTENAROMN : array[1..9]of Word = (100,200,300,400,500,600,700,800,900);
 var
   i,NU : Word;
   N : String;
   AntN,notIq : boolean;
begin
 ...
 NU := 0;
 N := '';
  while(i <= longitud)AND(UpCase(caracteres[i]) in ['C','D','M'])do
   begin
    if(UpCase(caracteres[i]) in ['C','D'])then
      AntN := true;
    if(AntN)then
      N := N + UpCase(caracteres[i]);
    i := i + 1;
 ...
 i := 1;
 notIq := false;
  while(i <= 9)AND not(notIq)do
   begin
     if(N = CENTENAROM[i])then
      begin
       NU := CENTENAROMN[i];
       notIq := true;
      end;
...
function Cadena.ObtDecenaRomano(Romano: String): Word;
const
 DECENAROM : array[1..9]of String = ('X','XX','XXX','XL','L','LX','LXX','LXXX','XC');
 DECENAROMN : array[1..9]of byte = (10,20,30,40,50,60,70,80,90);
 var
   i,NU : Word;
   N : String;
   AntN,notIq : boolean;
begin
 ...
 NU := 0;
 AntN := false;
  while(i <= longitud)AND not(UpCase(caracteres[i]) in ['I','V'])do
   begin
    if(UpCase(caracteres[i]) in ['X','L'])then
      AntN := true;
    if(AntN)then
      N := N + UpCase(caracteres[i]);
    i := i + 1;
...
function Cadena.ObtUnidadRomano(Romano: String): Word;
const
 UNIDADROM : array[1..9]of String = ('I','II','III','IV','V','VI','VII','VIII','IX');
 UNIDADROMN : array[1..9]of word = (1,2,3,4,5,6,7,8,9);
var
  i,NU : Word;
  N : String;
  AntN,notIq : boolean;
begin
 ...
 AntN := false;
  while(i <= longitud)do
   begin
    if(UpCase(caracteres[i]) in ['I','V'])then
      AntN := true;
    if(AntN)then
     N := N + UpCase(caracteres[i]);
    i := i + 1;
...
function Cadena.RomanoToDecimal(Romano: String): Cardinal;
 var
  Numero : Cardinal;
  NextN : Word;
begin
 numero := 0;
 NextN := ObtMillarRomano(getCadena);
 Numero := Numero + NextN;
 NextN := ObtCentenaRomano(getCadena);
 Numero := Numero + NextN;
 NextN := ObtDecenaRomano(getCadena);
 Numero := Numero + NextN;
 NextN := ObtUnidadRomano(getCadena);
 Numero := Numero + NextN;
 Result := Numero;
end;

function Cadena.LiteralToDecimal(): Cardinal;
 var
  Numero : Cardinal;
  NextN : Word;
begin
 Numero := 0;
 Numero := Numero + CentenaLiteal;
 Numero := Numero + DecenaLiteral;
 Numero := Numero + ExtraLiteral;
 Numero := Numero + UnidadLiteral;
 Result := Numero;
end;

function Cadena.CentenaLiteal(): Word;
const
 LITCENTENA : Array[1..9] of String =
  ('ciento','doscientos','trescientos','cuatrocientos','quinientos','seiscientos','setecientos','ochocientos','novecientos');
 CENTENAN : Array[1..9] of Word = (100,200,300,400,500,600,700,800,900);
 var
  i,NU,j : Word;
  fin : boolean;
begin
 ...
 NU := 0;
 fin := false;
 while(i <= CantDPal)AND not(fin)do
  begin
   j := 1;
    while(j <= 9)AND not(fin)do
     begin
      if(UpperCase(getPalXPos(i)) = UpperCase(LITCENTENA[j]))then
       begin
         fin := true;
         NU := CENTENAN[j];
       end;
...
function Cadena.DecenaLiteral(): Word;
const
 LITDECENA : Array[1..9] of String =
  ('diez','veinte','treinta','cuarenta','cincuenta','sesenta','setenta','ochenta','noventa');
 DECENAN : array[1..9] of Word = (10,20,30,40,50,60,70,80,90);
 var
  i,NU,j : Word;
  fin : boolean;
begin
 ...
 while(i <= CantDPal)AND not(fin)do
  begin
   j := 1;
    while(j <= 9)AND not(fin)do
     begin
      if(UpperCase(getPalXPos(i)) = UpperCase(LITDECENA[j]))then
       begin
         fin := true;
         NU := DECENAN[j];
       end;
...
function Cadena.UnidadLiteral(): Word;
const
  LITUNIDAD : Array[1..9] of string =
  ('uno','dos','tres','cuatro','cinco','seis','siete','ocho','nueve');
  UNIDADN : Array[1..9] of Word = (1,2,3,4,5,6,7,8,9);
 var
  i,NU,j : Word;
  fin : boolean;
begin
 ...
 while(i <= CantDPal)AND not(fin)do
  begin
   j := 1;
    while(j <= 9)AND not(fin)do
     begin
      if(UpperCase(getPalXPos(i)) = UpperCase(LITUNIDAD[j]))then
       begin
         fin := true;
         NU := UNIDADN[j];
       end;
...
function Cadena.ExtraLiteral: Word;
const
 LITEXTRA : Array[1..18] of String =
 ('Once','Doce','Trece','Catorce','Quince','Dieciseis','Diecisiete','Dieciocho','Diecinueve',
 'Veintiuno','Veintidos','Veintitres','Veinticuatro','Veinticinco','Veintiseis','Veintisiete','Veintiocho','Veintinueve');
 EXTRASN : Array[1..18] of Word = (11,12,13,14,15,16,17,18,19,21,22,23,24,25,26,27,28,29);
 var
  i,NU,j : Word;
  fin : boolean;
begin
 ...
 while(i <= CantDPal)AND not(fin)do
  begin
   j := 1;
    while(j <= 18)AND not(fin)do
     begin
      if(UpperCase(getPalXPos(i)) = UpperCase(LITEXTRA[j]))then
       begin
         fin := true;
         NU := EXTRASN[j];
       end;
