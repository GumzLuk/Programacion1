...
interface
 ...
 //funciones____________________________
      ...
      function getCantDig2(num : Cardinal): byte;
      ...
      function DeciToRomano(n: cardinal):String;
      ...
      function  ToUnidad(n : byte) : String;
      function  ToDecenas(n : byte) : String;
      function  ToCentenas(n : word) : String;
      function  ToLiteral(n : cardinal) : String;
      function  VerifPartFracc(x: real): boolean;
      function ParDDigSuman10():byte;  //Examen 3
      function SacarTodoDigX(dig:byte):Cardinal;
     function redondeo(N:Cardinal;Nro:byte):Cardinal;
   end;
...
begin
 ...
 k := false;
  while(i <= getCantDig)AND NOT(k)do
   begin
      if(i+getCantDig2(subN)-1 <= getCantDig)AND(SacarNXPos(i,i+getCantDig2(subN)-1) = subN)then
       begin
         k := true;
         j := 1;
          while(j <= getCantDig2(subN))do
           begin
             delDigit(i);
...
begin
 ...
 while(i <= getCantDig)do
  begin
    if(i+getCantDig2(SubN)-1 > getCantDig)AND(getPosDig(i) = subN mod 10)then
        SacarNXPos(i,i+getCantDig2(SubN)-1);
  end;
end;
...
function Numero.getCantDig2(num: Cardinal): byte;
begin
...
begin
 ...
 if(cont = 2)then Result := true else Result := false;
end;
...
function Numero.DeciToRomano(n: cardinal):String;
  const
     ROM:array[1..4,1..10]of String=(
     ('I','II','III','IV','V','VI','VII','VIII','IX',''),
     ('X','XX','XXX','XL','L','LX','LXX','LXXX','XC',''),
     ('C','CC','CCC','CD','D','DC','DCC','DCCC','CM',''),
     ('M','MM','MMM','F..','F','FF','FFF','FFFF','FFFFF',''));
   var
     p,d:byte;
     R:String;
begin
...
begin
 ...
 if(nf<=getValor())then
  begin
   while(i<=getCantDig())do
     begin
        if(getPosDig(i) = getPosDig2(nf,1))AND((i+getCantDig2(nf)-1)<=getCantDig())then begin
          if(SacarNXPos(i,i+getCantDig2(nf)-1)) = nf then
             cont := cont + 1;
        end;
      i := i + 1;
     end;
...
function Numero.ToUnidad(n: byte): String;
...
function Numero.ToDecenas(n: byte): String;
const
...
begin
  if(n>10)and(n<20)then
    ...
    Result := (DEC[1, n div 10] + r + ToUnidad(n mod 10));
  end;
end;

function Numero.ToCentenas(n: word): String;//300 => 3
...
begin
  ...
  if n<100 then
    ...
    r := 'to';
  Result := (CEN[n div 100] + r + ToDecenas(n mod 100));
end;

function Numero.ToLiteral(n: cardinal): String;
...
begin
  ...
  while n > 0 do  begin
    d := n mod 1000;
    n := n div 1000;
    r := ToCentenas(d) + VEC[p] + r;
    p := p + 1;
  end;
  result := r;
end;

function Numero.VerifPartFracc(x: real): boolean;
begin
...
function Numero.redondeo(N:Cardinal;Nro: byte): Cardinal;
var i:byte;
    k,n1,n2,nr1,nr2,F:Cardinal;
    R:Cardinal;
begin
 if(Nro < getCantDig2(N))then
  begin
    R := 0;
    k := trunc(Power(10,getCantDig2(N)-Nro));
    n1 := N div k;
    n2 := N mod k;
    nr1 := n1 mod 10;
    nr2 := n2 div trunc(Power(10,getCantDig2(n2)-1));
    F := trunc(Power(10,getCantDig2(n2)));
     if(nr2 >= 6)then R := F * (n1 + 1) else R := F*n1 + n2;
    Result := R;
   end else begin
     Result := N;
    end;
