...
procedure Matriz.SerieSerpienteR;
 var
  posx,posy,x,x1,x2,n,filN,colN,a,f,c,i : Word;
  ban : boolean;
begin
 a := 1;
 n := NroFilas + NroColumnas - 1;
 x1 := 1;
 x2 := 1;
 ban := true;
 filN := 1;
 colN := 1;
  for x := 1 to n do
   begin
    ...
    if(filN = NroFilas)then
      x1 := x1 + 1;
     if(colN = NroColumnas)then
      x2 := x2 + 1;
    if(filN < NroFilas)then
      filN := filN + 1;
     if(colN < NroColumnas)then
...
var
 ...
 NMayA,NMayB : Real;
begin
 NMayA := sacarNumeroFilaInicio(1);
  for i := 2 to NroFilas do
   begin
     if(NMayA < sacarNumeroFilaInicio(i))then
       NMayA := sacarNumeroFilaInicio(i);
   end;
 NMayB := sacarNumeroFilaFinal(1);
  for i := 2 to NroFilas do
    begin
      if(NMayB < self.sacarNumeroFilaFinal(i))then
        NMayB := self.sacarNumeroFilaFinal(i);
    end;
 if(NMayA >= NMayB)then
  Result := NMayA
    else Result := NMayB;
...
function Matriz.MaxNColumnas: Real;
 var
   ...
   NMayA,NmayB : Real;
begin
 NMayA := sacarNumeroColumnaArriba(1);
   for j := 2 to NroColumnas do
    begin
      if(NMayA < self.sacarNumeroColumnaArriba(j))then
          NMayA := self.sacarNumeroColumnaArriba(j);
    end;
 NmayB := self.sacarNumeroColumnaAbajo(1);
  for j := 2 to NroColumnas do
   begin
     if(NmayB < self.sacarNumeroColumnaAbajo(j))then
       NmayB := self.sacarNumeroColumnaAbajo(j);
   end;
 if(NMayA >= NmayB)then
  Result := NMayA
    else Result := NmayB;
...
function Matriz.MaxNMatriz: Real;
begin
 if(self.MaxNFilas > Self.MaxNColumnas)then
  Result := self.MaxNFilas
    else   Result := self.MaxNColumnas;
end;
