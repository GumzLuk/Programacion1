...
interface
 uses SysUtils, Dialogs, UCCadena, UCNumero;
 ...
 //setters______________________________________________________________________
          procedure addElement(elem: Real);
          procedure setDimension(dimN : Word);
          procedure insertElementIzquierda(posE:Word ; element:Real);
          procedure insertElementDerecha(posE:Word; element :Real);
          ...
          procedure MSortReverse(i,f:Word);
          procedure MergeSortReverse();
          ...
          procedure pasarXParametroNuevosElems(NuevosElems:String);
...
procedure Vector.insertElementDerecha(posE: Word; element: Real);
 var
  i : Word;
begin
 if(posE > 0)AND(posE <= dimension)then
  begin
    self.dimension := self.dimension + 1;
     for i := dimension downto posE+2 do
      begin
       elementos[i] := elementos[i-1];
      end;
    elementos[posE+1] := element;
  end;
end;

procedure Vector.insertElementIzquierda(posE: Word; element: Real);
var
  i : Word;
begin
 if(posE > 0)AND(posE <= dimension)then
...
procedure Vector.MSortReverse(i, f: Word);
var m:Word;
begin
 if(i < f)then
  begin
...
procedure Vector.MergeSortReverse;
begin
 MSortReverse(1,dimension);
end;
...
procedure Vector.pasarXParametroNuevosElems(NuevosElems: String);
 var
  CadenaX : Cadena;
  i,j : Word;
  elem : Real;
...
begin
 ...
 CadenaX.cargarTexto(NuevosElems);
 i := 1;
  while(i <= CadenaX.CantDPal)do
   begin
     elem := StrToFloat(CadenaX.getPalXPos(i));
     j := Self.dimension;
     cambio := false;
       while(j >= 1)AND not(cambio)do
        begin
          if(elem >= elementos[j])then
            begin
              cambio := true;
            end;
          j := j - 1;
     ...
     if(cambio)then
       insertElementDerecha(j+1,elem)
         else insertElementIzquierda(1,elem);
     i := i + 1;
   end;
end;
...
function Vector.ObtDate: TDateTime;
var
 dat1,dat2 : TDateTime;
begin
 dat1 := StrToDateTime('05:59:04');
 dat2 :=StrToDateTime('04:59:04');
 if(dat1 > dat2)then
  Result := dat1
   else Result := dat2;
end;
