unit UCVectStr;     //Ord(char) -funcion : codigo ascii
...
begin
 if(posE > 0)AND(posE <= dimension)then
  begin
   cad := Cadena.Create;
   cad.cargarTexto(elementos[posE]);
   cad.cargarTexto(cad.getPalXPos(1));
   elementos[posE] := cad.getCadena;
  end;
end;
    ...
    j := 2;
     if(cad.CantDPal > 1)then
      begin
       ...
       EliminarElementMayorUnString(i);
      end;
    i := i + 1;
   end;
end;

procedure vectorSTR.EliminarElemenRep;
 var
  i,j : Word;
begin
 i := 1;
   ...
   begin
    j := i + 1;
     while(j <= dimension)do
      begin
         if(UpperCase(elementos[i]) = UpperCase(elementos[j]))then
          begin
            elimElement(j);
          end else  j := j + 1;
      end;
